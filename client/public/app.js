  ...
  $.getJSON("/all", function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      $("#results").prepend("<p class='dataentry' data-id=" + data[i]._id + "> <span class='dataTitle' data-id=" +
        data[i]._id + ">" + data[i].email + "</span><span class=deleter>X</span>" +data[i].lastname + data[i].firstname + data[i].textarea+"</p>");
    }
  });
}
...
$(document).on("click", "#submit", function() {
  ...
  .done(function(data) {
    ...
    $("#contactInfo").prepend("<p class='dataentry' data-id=" + data._id + "> <span class='dataTitle' data-id=" +
    data._id + ">" + "<p> Name: " + data.firstname + " " + data.lastname + "</p>" + "<p> Email: " + data.email + "</p>" + "<p> Comments: " + data.textarea +
    "</span>"); 
...
$(document).on("click", ".deleter", function() {
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      $("#actionbutton").html("<button id='submit'>Submit</button>");
    }
  });
});
...
$(document).on("click", ".dataTitle", function() {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      $("#submit").html("<button id='updater' data-id='" + data._id + "'>Update</button>");
    }
  });
});
...
$(document).on("click", "#updater", function() {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      $("#submit").html("<button id='makenew'>Submit</button>");
